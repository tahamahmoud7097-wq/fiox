...
pub fn csv_writer(data: &UniversalData, path: &PathBuf) {
    ...
    if let UniversalData::Table { headers, rows } = data {
        ...
        let mut wtr =
            csv::Writer::from_path(path).better_expect("ERROR: Failed to open output CSV file.");
        // Write headers into the file.
        wtr.write_record(headers)
            .better_expect("ERROR: Failed to write CSV file headers.");
        // Loop for writing rows into the file.
        for row in rows {
            wtr.write_record(row)
                .better_expect("ERROR: Failed to write CSV file rows.");
        }
        wtr.flush()
            .better_expect("ERROR: Failed to flush final CSV.");
